"""
app.py — KINO Agent με Supabase PostgreSQL
"""
...
def save_draw(draw_id, draw_time, numbers):
    ...
    c.execute("INSERT INTO draws VALUES (%s,%s,%s,%s) ON CONFLICT (draw_id) DO NOTHING",
              (draw_id, draw_time, json.dumps(numbers), datetime.now().isoformat()))
    conn.commit(); conn.close()

def get_draws(n=500):
    conn = get_conn(); c = conn.cursor()
    c.execute("SELECT draw_id,draw_time,numbers FROM draws ORDER BY draw_id DESC LIMIT %s", (n,))
...
def opap_get(url):
    s = requests.Session(); s.headers.update(OPAP_HEADERS)
...
def fetch_latest():
    ...
    last = data.get("last", {})
    did = last.get("drawId"); dt = str(last.get("drawTime",""))
    nums = last.get("winningNumbers",{}).get("list",[])
    if did and nums: save_draw(did, dt, nums); return did
    return None

def fetch_history(days=7):
    loaded = 0; today = datetime.now().date()
    for d in range(days, -1, -1):
        ds = (today - timedelta(days=d)).strftime("%Y-%m-%d")
        try:
            data = opap_get(f"https://api.opap.gr/draws/v3.0/1100/draw-date/{ds}/{ds}")
            for item in data.get("content",[]):
                did = item.get("drawId"); nums = item.get("winningNumbers",{}).get("list",[])
                if did and nums: save_draw(did, str(item.get("drawTime","")), nums); loaded += 1
            time.sleep(0.3)
        except Exception as e: log.warning(f"History {ds}: {e}")
    return loaded

def predict(draws, n=PREDICT_N):
    if len(draws) < 20: return list(range(1, n+1))
    recent = draws[-200:]
    ...
    mf = max(freq.values()) or 1; mg = max(gaps.values()) or 1
    N = n*3
...
def eval_stats(draws, sample=200):
    results = []
    for i in range(25, min(len(draws), 25+sample)):
        p = predict(draws[:i]); hits = len(set(p) & set(draws[i]["numbers"]))
...
def background_loop():
    ...
    try:
        if count_draws() < 50:
            n = fetch_history(7); log.info(f"Loaded {n}. Total: {count_draws()}")
    except Exception as e: log.error(f"Bootstrap: {e}")
    while True:
        try:
            did = fetch_latest()
            if did: log.info(f"Draw #{did}. Total: {count_draws()}")
        except Exception as e: log.warning(f"Fetch: {e}")
        time.sleep(120)
...
@app.route("/api/state")
def state():
    draws = get_draws(500); pred = predict(draws); stats = eval_stats(draws)
    return jsonify({
        "draws": draws[-100:], "predictions": pred,
        "next_draw_id": (draws[-1]["draw_id"] if draws else 0)+1,
        "results": stats[-200:], "total": count_draws(),
        "server_time": datetime.now().isoformat(),
    })

@app.route("/health")
def health(): return jsonify({"status":"ok","draws":count_draws()})
